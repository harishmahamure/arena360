...
.DESCRIPTION
  Registers the watchdog scheduled task, optional HKLM policy hardening, and optional
  ...
  Launches Arena360 at logon via the watchdog scheduled task only. Does not modify
  Winlogon\Shell or replace explorer.exe as the Windows shell.

.PARAMETER Uninstall
  Remove watchdog task and registry values recorded in the Arena360 marker file.
#>
[CmdletBinding()]
param(
...
function Resolve-WatchdogExe([string]$Dir) {
    ...
    $exact = Join-Path $Dir 'arena360-watchdog.exe'
    if (Test-Path -LiteralPath $exact) {
        return $exact
    }
    $match = Get-ChildItem -LiteralPath $Dir -Filter 'arena360-watchdog*.exe' -ErrorAction SilentlyContinue |
...
function Remove-WatchdogTask {
    $null = schtasks /Delete /TN $WatchdogTaskName /F 2>&1
    Write-Info "Removed watchdog scheduled task (if present)."
}
...
function Invoke-Uninstall {
    ...
    if (Test-Path -LiteralPath (Join-Path $MarkerDir 'watchdog.pause')) {
        Remove-Item -LiteralPath (Join-Path $MarkerDir 'watchdog.pause') -Force
    }
...
if ($ShellMode -eq 'ReplaceShell') {
    ...
    Write-Warn 'Run-key shell mode is optional; the watchdog scheduled task launches the kiosk at logon.'
}

if (-not $SkipWatchdog) {
    if ([string]::IsNullOrWhiteSpace($InstallDir)) {
        Write-Warn 'InstallDir not provided; skipping watchdog task.'
    } else {
