...
#[cfg(windows)]
static INSTANCE_MUTEX: Mutex<Option<crate::watchdog::win::InstanceMutexGuard>> = Mutex::new(None);
...
pub fn init_instance_mutex() {
    #[cfg(target_os = "windows")]
    {
        if let Ok(guard) = crate::watchdog::win::acquire_instance_mutex() {
            if let Ok(mut slot) = INSTANCE_MUTEX.lock() {
                *slot = Some(guard);
            }
        }
    }
