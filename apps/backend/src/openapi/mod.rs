use utoipa::openapi::security::{HttpAuthScheme, HttpBuilder, SecurityScheme};
use utoipa::{Modify, OpenApi};

use crate::dto::auth_dto::{
    ActiveSessionDto, AuthResponseDto, AuthUserDto, ChangePasswordDto, KioskRegisterDto,
    KioskRegisterResponseDto, LoginDto, PanelLoginResponseDto, PanelMfaDto, RegisterDto,
    RegisterResponseDto, StaffLoginDto,
};
use crate::dto::{ApproveInventoryActionDto, ReceiptSummaryFilterDto, WasteSummaryFilterDto};
use crate::handlers;
use crate::models::{
    ActivityLog, ActivityLogFilterDto, ApproveDepositDto, ApproveExpenseDto, AssignPlanDto,
    CashDeposit, CashDepositFilterDto, CashRegister, CashRegisterEntry, CashRegisterFilterDto,
    CashRegisterWithEntries, ClockInDto, ClockOutDto, CloseCashRegisterDto, ConfigFilterDto,
    Configuration, ConvertKioskOrderDto, CreateCashRegisterEntryDto, CreateDeviceDto,
    CreateExpenseCategoryDto, CreateExpenseDto, CreateInventoryLocationDto, CreateKioskOrderDto,
    CreateKioskOrderLineItemDto, CreateLineItemDto, CreatePlanDto, CreateProductDto,
    CreatePurchaseOrderDto, CreateSessionDto, CreateStockAdjustmentDto, CreateStockReceiptDto,
    CreateStockTransferRequestDto, CreateStockWasteEventDto, CreateTransactionDto, CreateUnitDto,
    CreateVendorDto, CreditAccountFilterDto, CreditLastSettlement, CreditPlayerRow,
    CreditPortfolioSummary, CreditSettlement, CreditSettlementDetail, CreditSettlementFilterDto,
    CreditSettlementItemRow, CreditSettlementListRow, CreditSummary, Device, DeviceFilterDto,
    EndSessionDto, Expense, ExpenseCategory, ExpenseCategoryFilterDto, ExpenseFilterDto,
    ExpenseSummaryDto, HandoverDepositDto, InitiateDepositDto, InventoryLocation,
    InventoryLocationFilterDto, InventoryOverviewDto, InventoryReorderRule, KioskMenuProduct,
    KioskOrderFilterDto, KioskOrderItemResponse, KioskOrderWithItems, LocationStockFilterDto,
    LocationStockRow, NotificationFilterDto, NotificationItem, OpenCashRegisterDto,
    OutstandingTxnRow, Plan, PlanFilterDto, PlayerCreditDetail, PlayerPlan, PlayerPlanFilterDto,
    Product, ProductFilterDto, PurchaseOrder, PurchaseOrderFilterDto, PurchaseOrderLine,
    PurchaseOrderLineInput, PurchaseOrderWithLines, ReceiptSummaryRow, ReceivePurchaseOrderDto,
    ReceivePurchaseOrderResponse, ReconcileCashRegisterDto, RejectDepositDto, RejectExpenseDto,
    RejectPurchaseOrderDto, RejectStockTransferDto, RejectStockWasteDto, ReorderSuggestion,
    SessionFilterDto, SetCreditLimitDto, SetStaffGamingAllowanceDto, SettleCreditDto,
    SettleItemDto, Shift, ShiftCloseDto, ShiftCloseResponseDto, ShiftFilterDto, ShiftHandoverDto,
    ShiftHandoverResponseDto, ShiftStartContextDto, ShiftStartResponseDto,
    StaffGamingAllowanceStatus, StaffGamingAllowanceSummary, StartShiftDto, StockAdjustment,
    StockAdjustmentFilterDto, StockAdjustmentWithLines, StockMovementFilterDto, StockMovementRow,
    StockReceipt, StockReceiptFilterDto, StockReceiptWithLines, StockTransferFilterDto,
    StockTransferRequest, StockTransferRequestWithLines, StockWasteEvent, StockWasteEventWithLines,
    StockWasteFilterDto, TotpSetupResponseDto, Transaction, TransactionFilterDto,
    TransactionProductResponse, TransactionResponse, TransactionWithLineItems, Unit, UnitFilterDto,
    UnreadCountDto, UpdateDeviceDto, UpdateDeviceStatusDto, UpdateExpenseCategoryDto,
    UpdateExpenseDto, UpdateInventoryLocationDto, UpdateKioskOrderDto, UpdateOpeningBalanceDto,
    UpdatePlanDto, UpdateProductDto, UpdatePurchaseOrderDto, UpdateTransactionDto, UpdateUnitDto,
    UpdateUserDto, UpdateVendorDto, UpsertConfigDto, UpsertInventoryReorderRuleDto, UsageSession,
    UsageSessionResponse, User, UserFilterDto, ValidationResult, Vendor, VendorFilterDto,
    VerifyTotpSetupDto, WasteSummaryRow,
};
use crate::openapi::responses::*;
use crate::realtime::rooms::{AddMemberDto, CreateRoomDto, Room};
use crate::services::stats_service::{
    DashboardStatsDto, DeviceStatsDto, FinanceDepositMetricsDto, FinanceDepositStatsDto,
    FinanceReconciliationMetricsDto, FinanceReconciliationStatsDto, FinanceVarianceMetricsDto,
    FinanceVarianceRegisterRow, FinanceVarianceStatsDto, PeriodDto,
    PeriodPairRevenueByPaymentMethod, PeriodPairUsageStats, PlanStatsDto, PlanTypeStat,
    RevenueByPaymentMethodDto, RevenueTrendDto, StaffDashboardStatsDto, StaffDeviceStatsDto,
    StaffPlayerStatsDto, TopPerformersDto, TransactionStatsDto, UsageStatsDto, UserStatsDto,
};

pub mod responses;

struct SecurityAddon;

impl Modify for SecurityAddon {
    fn modify(&self, openapi: &mut utoipa::openapi::OpenApi) {
        let components = openapi.components.get_or_insert_with(Default::default);
        components.add_security_scheme(
            "bearer_auth",
            SecurityScheme::Http(
                HttpBuilder::new()
                    .scheme(HttpAuthScheme::Bearer)
                    .bearer_format("JWT")
                    .description(Some(
                        "JWT obtained from POST /auth/login/panel (legacy admin/staff login wrappers remain available during migration)"
                            .to_string(),
                    ))
                    .build(),
            ),
        );
    }
}

#[derive(OpenApi)]
#[openapi(
    info(
        title = "Game Zone Management System API",
        version = "1.0.0",
        description = "REST API for managing a gaming cafe: users, devices, plans, sessions, transactions, and inventory."
    ),
    servers(
        (url = "http://localhost:3000", description = "Local development server"),
    ),
    paths(
        handlers::health::live_check,
        handlers::health::ready_check,
        handlers::health::health_check_legacy,
        handlers::auth::login_admin,
        handlers::auth::login_panel,
        handlers::auth::verify_panel_mfa,
        handlers::auth::login_staff,
        handlers::auth::login_player,
        handlers::auth::register_player,
        handlers::auth::register,
        handlers::stats::dashboard_stats,
        handlers::stats::staff_dashboard_stats,
        handlers::stats::revenue_by_payment_method,
        handlers::stats::usage_stats,
        handlers::stats::finance_reconciliation_stats,
        handlers::stats::finance_deposit_stats,
        handlers::stats::finance_variance_stats,
        handlers::users::list_users,
        handlers::users::get_user,
        handlers::users::update_user,
        handlers::users::change_password,
        handlers::users::setup_totp,
        handlers::users::verify_totp_setup,
        handlers::users::disable_totp,
        handlers::devices::list_devices,
        handlers::devices::get_device,
        handlers::devices::create_device,
        handlers::devices::update_device,
        handlers::devices::update_device_status,
        handlers::devices::delete_device,
        handlers::kiosk::start_session,
        handlers::kiosk::current_session,
        handlers::kiosk::heartbeat_session,
        handlers::kiosk::end_session,
        handlers::kiosk_orders::list_products,
        handlers::kiosk_orders::place_order,
        handlers::kiosk_orders::current_order,
        handlers::kiosk_orders::list_orders,
        handlers::kiosk_orders::get_order,
        handlers::kiosk_orders::update_order,
        handlers::kiosk_orders::convert_order,
        handlers::plans::list_plans,
        handlers::plans::get_active_plans,
        handlers::plans::get_plan,
        handlers::plans::create_plan,
        handlers::plans::update_plan,
        handlers::plans::delete_plan,
        handlers::balances::list_balances,
        handlers::balances::list_my_active_balances,
        handlers::balances::get_best_balance,
        handlers::balances::purchase_balance,
        handlers::balances::get_balance,
        handlers::balances::validate_access,
        handlers::units::list_units,
        handlers::units::get_unit,
        handlers::units::create_unit,
        handlers::units::update_unit,
        handlers::units::delete_unit,
        handlers::sessions::list_sessions,
        handlers::sessions::list_active_sessions,
        handlers::sessions::get_session,
        handlers::sessions::create_session,
        handlers::sessions::end_session,
        handlers::transactions::list_transactions,
        handlers::transactions::get_transaction,
        handlers::transactions::create_transaction,
        handlers::transactions::update_transaction,
        handlers::products::list_products,
        handlers::products::get_product,
        handlers::products::create_product,
        handlers::products::update_product,
        handlers::products::delete_product,
        handlers::games::list_games,
        handlers::games::get_game,
        handlers::games::create_game,
        handlers::games::update_game,
        handlers::games::delete_game,
        handlers::uploads::presign_upload,
        handlers::devices::provision_device,
        handlers::realtime_rooms::list_rooms,
        handlers::realtime_rooms::create_room,
        handlers::realtime_rooms::add_member,
        handlers::realtime_rooms::remove_member,
        handlers::config::list_config,
        handlers::config::get_config,
        handlers::config::upsert_config,
        handlers::config::settings_catalog,
        handlers::config::venue_locations,
        handlers::config::effective_settings,
        handlers::config::upsert_setting_override,
        handlers::config::delete_setting_override,
        handlers::config::setting_history,
        handlers::config::configuration_snapshot,
        handlers::pricing_rules::list_rule_sets,
        handlers::pricing_rules::create_rule_set,
        handlers::pricing_rules::list_versions,
        handlers::pricing_rules::create_version,
        handlers::pricing_rules::validate_version,
        handlers::pricing_rules::simulate_version,
        handlers::pricing_rules::publish_version,
        handlers::pricing_rules::rollback_version,
        handlers::shifts::clock_in,
        handlers::shifts::start_context,
        handlers::shifts::start_shift,
        handlers::shifts::clock_out,
        handlers::shifts::handover_shift,
        handlers::shifts::close_shift,
        handlers::shifts::get_active_shift,
        handlers::shifts::list_shifts,
        handlers::shifts::get_shift,
        handlers::shifts::force_close_shift,
        handlers::cash_registers::open_cash_register,
        handlers::cash_registers::close_cash_register,
        handlers::cash_registers::reconcile_cash_register,
        handlers::cash_registers::update_opening_balance,
        handlers::cash_registers::add_entry,
        handlers::cash_registers::get_cash_register,
        handlers::cash_registers::list_cash_registers,
        handlers::cash_registers::get_active_expected_closing,
        handlers::cash_deposits::initiate_deposit,
        handlers::cash_deposits::list_deposits,
        handlers::cash_deposits::get_deposit,
        handlers::cash_deposits::approve_deposit,
        handlers::cash_deposits::reject_deposit,
        handlers::expense_categories::list_expense_categories,
        handlers::expense_categories::get_expense_category,
        handlers::expense_categories::create_expense_category,
        handlers::expense_categories::update_expense_category,
        handlers::expense_categories::delete_expense_category,
        handlers::vendors::list_vendors,
        handlers::vendors::get_vendor,
        handlers::vendors::create_vendor,
        handlers::vendors::update_vendor,
        handlers::vendors::delete_vendor,
        handlers::expenses::list_expenses,
        handlers::expenses::get_expense,
        handlers::expenses::create_expense,
        handlers::expenses::update_expense,
        handlers::expenses::approve_expense,
        handlers::expenses::reject_expense,
        handlers::expenses::delete_expense,
        handlers::expenses::expense_summary,
        handlers::credit::list_credit_accounts,
        handlers::credit::credit_summary,
        handlers::credit::get_player_credit,
        handlers::credit::list_settlements,
        handlers::credit::get_settlement,
        handlers::credit::create_settlement,
        handlers::credit::update_credit_limit,
        handlers::staff_gaming_allowance::get_staff_gaming_allowance,
        handlers::staff_gaming_allowance::update_staff_gaming_allowance,
        handlers::inventory::list_locations,
        handlers::inventory::create_location,
        handlers::inventory::update_location,
        handlers::inventory::list_stock,
        handlers::inventory::create_receipt,
        handlers::inventory::list_receipts,
        handlers::inventory::create_adjustment,
        handlers::inventory::list_adjustments,
        handlers::inventory::get_adjustment,
        handlers::inventory::create_transfer_request,
        handlers::inventory::list_transfer_requests,
        handlers::inventory::get_transfer_request,
        handlers::inventory::approve_transfer_request,
        handlers::inventory::reject_transfer_request,
        handlers::inventory::fulfill_transfer_request,
        handlers::inventory::create_waste_event,
        handlers::inventory::list_waste_events,
        handlers::inventory::get_waste_event,
        handlers::inventory::approve_waste_event,
        handlers::inventory::reject_waste_event,
        handlers::inventory::waste_summary,
        handlers::inventory::receipt_summary,
        handlers::procurement::overview,
        handlers::procurement::list_movements,
        handlers::procurement::list_orders,
        handlers::procurement::get_order,
        handlers::procurement::create_order,
        handlers::procurement::update_order,
        handlers::procurement::submit_order,
        handlers::procurement::approve_order,
        handlers::procurement::reject_order,
        handlers::procurement::mark_ordered,
        handlers::procurement::cancel_order,
        handlers::procurement::receive_order,
        handlers::procurement::list_reorder_rules,
        handlers::procurement::upsert_reorder_rule,
        handlers::procurement::reorder_suggestions,
        handlers::notifications::list_notifications,
        handlers::notifications::unread_count,
        handlers::notifications::mark_read,
        handlers::notifications::mark_all_read,
        handlers::notifications::list_activity_log,
    ),
    components(
        schemas(
            handlers::health::HealthData,
            handlers::health::LiveHealthData,
            handlers::stats::StatsQuery,
            handlers::stats::StaffStatsQuery,
            ErrorEnvelope,
            LegacyHealthResponse,
            HealthEnvelope,
            LiveHealthEnvelope,
            LoginDto,
            StaffLoginDto,
            PanelMfaDto,
            PanelLoginResponseDto,
            crate::dto::PlayerLoginDto,
            crate::dto::DeviceFingerprintDto,
            crate::dto::RegisteredDeviceDto,
            crate::dto::DeviceRegisterResponseDto,
            crate::dto::StartKioskSessionDto,
            crate::dto::KioskSessionResponseDto,
            crate::models::deduction_profile::DeductionProfile,
            crate::dto::            EndKioskSessionDto,
            RegisterDto,
            KioskRegisterDto,
            KioskRegisterResponseDto,
            ChangePasswordDto,
            AuthResponseDto,
            AuthUserDto,
            ActiveSessionDto,
            AuthResponseEnvelope,
            PanelLoginResponseEnvelope,
            RegisterResponseDto,
            RegisterResponseEnvelope,
            KioskRegisterResponseEnvelope,
            DashboardStatsDto,
            DashboardStatsEnvelope,
            StaffDashboardStatsDto,
            StaffDashboardStatsEnvelope,
            StaffPlayerStatsDto,
            StaffDeviceStatsDto,
            PeriodDto,
            PeriodPairRevenueByPaymentMethod,
            RevenueByPaymentMethodEnvelope,
            PeriodPairUsageStats,
            UsageStatsEnvelope,
            FinanceReconciliationStatsDto,
            FinanceReconciliationMetricsDto,
            FinanceReconciliationStatsEnvelope,
            FinanceDepositStatsDto,
            FinanceDepositMetricsDto,
            FinanceDepositStatsEnvelope,
            FinanceVarianceStatsDto,
            FinanceVarianceMetricsDto,
            FinanceVarianceRegisterRow,
            FinanceVarianceStatsEnvelope,
            RevenueByPaymentMethodDto,
            TransactionStatsDto,
            UsageStatsDto,
            UserStatsDto,
            PlanStatsDto,
            PlanTypeStat,
            DeviceStatsDto,
            TopPerformersDto,
            RevenueTrendDto,
            Device,
            DeviceFilterDto,
            CreateDeviceDto,
            UpdateDeviceDto,
            UpdateDeviceStatusDto,
            DeviceEnvelope,
            DevicePaginationEnvelope,
            DevicePaginationPage,
            Plan,
            PlanFilterDto,
            CreatePlanDto,
            UpdatePlanDto,
            PlanEnvelope,
            PlanPaginationEnvelope,
            PlanPaginationPage,
            ActivePlansEnvelope,
            PlayerPlan,
            PlayerPlanResponse,
            PlayerPlanFilterDto,
            AssignPlanDto,
            ValidationResult,
            PlayerPlanEnvelope,
            PlayerPlanFlatEnvelope,
            PlayerPlanPaginationEnvelope,
            PlayerPlanPaginationPage,
            ValidationResultEnvelope,
            crate::models::PlayerPlanBalance,
            crate::models::PlayerPlanBalanceResponse,
            crate::models::BalanceFilterDto,
            crate::models::PurchaseBalanceDto,
            crate::models::BalanceValidationResult,
            crate::models::PlayerPlanLedger,
            BalanceEnvelope,
            BalanceFlatEnvelope,
            BalancePaginationEnvelope,
            BalancePaginationPage,
            BalanceValidationEnvelope,
            Unit,
            UnitFilterDto,
            CreateUnitDto,
            UpdateUnitDto,
            UnitEnvelope,
            UnitPaginationEnvelope,
            UnitPaginationPage,
            UsageSession,
            UsageSessionResponse,
            SessionFilterDto,
            CreateSessionDto,
            EndSessionDto,
            SessionEnvelope,
            SessionFlatEnvelope,
            SessionPaginationEnvelope,
            SessionPaginationPage,
            Transaction,
            TransactionResponse,
            TransactionFilterDto,
            CreateTransactionDto,
            UpdateTransactionDto,
            TransactionEnvelope,
            TransactionWithLineItems,
            TransactionWithLineItemsEnvelope,
            TransactionProductResponse,
            CreateLineItemDto,
            TransactionPaginationEnvelope,
            TransactionPaginationPage,
            Product,
            ProductFilterDto,
            CreateProductDto,
            UpdateProductDto,
            ProductEnvelope,
            ProductPaginationEnvelope,
            ProductPaginationPage,
            crate::models::Game,
            crate::models::CreateGameDto,
            crate::models::UpdateGameDto,
            crate::models::GameFilterDto,
            GameEnvelope,
            GamePaginationEnvelope,
            GamePaginationPage,
            crate::handlers::uploads::PresignRequest,
            crate::handlers::uploads::PresignResponse,
            PresignResponseEnvelope,
            crate::dto::ProvisionDeviceDto,
            User,
            UserFilterDto,
            UpdateUserDto,
            UserEnvelope,
            UserPaginationEnvelope,
            UserPaginationPage,
            Shift,
            ShiftFilterDto,
            ClockInDto,
            ClockOutDto,
            ShiftEnvelope,
            ShiftActiveEnvelope,
            ShiftPaginationEnvelope,
            ShiftPaginationPage,
            StartShiftDto,
            ShiftStartContextDto,
            ShiftStartResponseDto,
            ShiftStartContextEnvelope,
            ShiftStartResponseEnvelope,
            Configuration,
            ConfigFilterDto,
            UpsertConfigDto,
            ShiftHandoverDto,
            ShiftHandoverResponseDto,
            ShiftHandoverResponseEnvelope,
            ShiftCloseDto,
            ShiftCloseResponseDto,
            ShiftCloseResponseEnvelope,
            TotpSetupResponseDto,
            VerifyTotpSetupDto,
            TotpSetupEnvelope,
            ExpectedClosingEnvelope,
            CashDeposit,
            CashDepositFilterDto,
            InitiateDepositDto,
            ApproveDepositDto,
            RejectDepositDto,
            HandoverDepositDto,
            CashDepositEnvelope,
            CashDepositPaginationEnvelope,
            CashDepositPaginationPage,
            ConfigurationEnvelope,
            ConfigurationListEnvelope,
            CashRegister,
            CashRegisterEntry,
            CashRegisterWithEntries,
            CashRegisterFilterDto,
            OpenCashRegisterDto,
            CloseCashRegisterDto,
            ReconcileCashRegisterDto,
            UpdateOpeningBalanceDto,
            CreateCashRegisterEntryDto,
            CashRegisterEnvelope,
            CashRegisterWithEntriesEnvelope,
            CashRegisterEntryEnvelope,
            CashRegisterEntriesEnvelope,
            CashRegisterPaginationEnvelope,
            CashRegisterPaginationPage,
            ExpenseCategory,
            ExpenseCategoryFilterDto,
            CreateExpenseCategoryDto,
            UpdateExpenseCategoryDto,
            ExpenseCategoryEnvelope,
            ExpenseCategoryPaginationEnvelope,
            ExpenseCategoryPaginationPage,
            Vendor,
            VendorFilterDto,
            CreateVendorDto,
            UpdateVendorDto,
            VendorEnvelope,
            VendorPaginationEnvelope,
            VendorPaginationPage,
            Expense,
            ExpenseFilterDto,
            CreateExpenseDto,
            UpdateExpenseDto,
            ApproveExpenseDto,
            RejectExpenseDto,
            ExpenseSummaryDto,
            ExpenseEnvelope,
            ExpenseSummaryListEnvelope,
            ExpensePaginationEnvelope,
            ExpensePaginationPage,
            CreditSummary,
            CreditLastSettlement,
            CreditPortfolioSummary,
            CreditPlayerRow,
            OutstandingTxnRow,
            PlayerCreditDetail,
            CreditSettlement,
            CreditSettlementDetail,
            CreditSettlementFilterDto,
            CreditSettlementItemRow,
            CreditSettlementListRow,
            CreditAccountFilterDto,
            SettleCreditDto,
            SettleItemDto,
            SetCreditLimitDto,
            SetStaffGamingAllowanceDto,
            StaffGamingAllowanceSummary,
            StaffGamingAllowanceStatus,
            StaffGamingAllowanceSummaryEnvelope,
            CreditSummaryEnvelope,
            CreditPortfolioSummaryEnvelope,
            PlayerCreditDetailEnvelope,
            CreditSettlementEnvelope,
            CreditSettlementDetailEnvelope,
            CreditSettlementPaginationEnvelope,
            CreditSettlementPaginationPage,
            CreditPlayerPaginationEnvelope,
            CreditPlayerPaginationPage,
            Room,
            CreateRoomDto,
            AddMemberDto,
            InventoryLocation,
            InventoryLocationFilterDto,
            CreateInventoryLocationDto,
            UpdateInventoryLocationDto,
            InventoryLocationEnvelope,
            InventoryLocationPaginationEnvelope,
            InventoryLocationPaginationPage,
            LocationStockRow,
            LocationStockFilterDto,
            LocationStockPaginationEnvelope,
            LocationStockPaginationPage,
            InventoryOverviewDto,
            InventoryOverviewEnvelope,
            StockMovementRow,
            StockMovementFilterDto,
            StockMovementPaginationEnvelope,
            StockMovementPaginationPage,
            PurchaseOrder,
            PurchaseOrderLine,
            PurchaseOrderWithLines,
            PurchaseOrderLineInput,
            PurchaseOrderFilterDto,
            CreatePurchaseOrderDto,
            UpdatePurchaseOrderDto,
            RejectPurchaseOrderDto,
            ReceivePurchaseOrderDto,
            ReceivePurchaseOrderResponse,
            PurchaseOrderEnvelope,
            PurchaseOrderReceiptEnvelope,
            PurchaseOrderPaginationEnvelope,
            PurchaseOrderPaginationPage,
            InventoryReorderRule,
            UpsertInventoryReorderRuleDto,
            ReorderSuggestion,
            InventoryReorderRuleEnvelope,
            InventoryReorderRuleListEnvelope,
            ReorderSuggestionListEnvelope,
            StockReceipt,
            StockReceiptFilterDto,
            CreateStockReceiptDto,
            StockReceiptWithLines,
            StockReceiptEnvelope,
            StockReceiptWithLinesEnvelope,
            StockReceiptPaginationEnvelope,
            StockReceiptPaginationPage,
            ReceiptSummaryRow,
            ReceiptSummaryFilterDto,
            StockReceiptSummaryListEnvelope,
            StockAdjustment,
            StockAdjustmentFilterDto,
            CreateStockAdjustmentDto,
            StockAdjustmentWithLines,
            StockAdjustmentEnvelope,
            StockAdjustmentWithLinesEnvelope,
            StockAdjustmentPaginationEnvelope,
            StockAdjustmentPaginationPage,
            StockTransferRequest,
            StockTransferRequestWithLines,
            StockTransferFilterDto,
            CreateStockTransferRequestDto,
            RejectStockTransferDto,
            StockTransferEnvelope,
            StockTransferWithLinesEnvelope,
            StockTransferPaginationEnvelope,
            StockTransferPaginationPage,
            StockWasteEvent,
            StockWasteEventWithLines,
            StockWasteFilterDto,
            CreateStockWasteEventDto,
            RejectStockWasteDto,
            StockWasteEnvelope,
            StockWasteWithLinesEnvelope,
            StockWastePaginationEnvelope,
            StockWastePaginationPage,
            WasteSummaryRow,
            WasteSummaryFilterDto,
            ApproveInventoryActionDto,
            StockWasteSummaryListEnvelope,
            NotificationItem,
            NotificationFilterDto,
            UnreadCountDto,
            UnreadCountEnvelope,
            NotificationPaginationEnvelope,
            NotificationPaginationPage,
            ActivityLog,
            ActivityLogFilterDto,
            ActivityLogPaginationEnvelope,
            ActivityLogPaginationPage,
            KioskMenuProduct,
            KioskOrderWithItems,
            KioskOrderItemResponse,
            CreateKioskOrderDto,
            CreateKioskOrderLineItemDto,
            UpdateKioskOrderDto,
            ConvertKioskOrderDto,
            KioskOrderFilterDto,
        )
    ),
    modifiers(&SecurityAddon),
    tags(
        (name = "health", description = "Health and readiness probes"),
        (name = "auth", description = "Authentication"),
        (name = "stats", description = "Dashboard statistics"),
        (name = "users", description = "User management"),
        (name = "devices", description = "Device inventory"),
        (name = "plans", description = "Subscription plans"),
        (name = "player-plans", description = "Player plan assignments"),
        (name = "units", description = "Inventory units"),
        (name = "sessions", description = "Usage sessions"),
        (name = "kiosk", description = "Kiosk player session lifecycle"),
        (name = "kiosk-orders", description = "Kiosk player food/drink orders"),
        (name = "transactions", description = "Payment transactions"),
        (name = "products", description = "Products"),
        (name = "games", description = "Game media catalog"),
        (name = "uploads", description = "Object storage presigned uploads"),
        (name = "shifts", description = "Staff shift management"),
        (name = "cash-registers", description = "Cash register management"),
        (name = "credit", description = "Player credit accounts and settlements"),
        (name = "config", description = "Business configuration"),
        (name = "expense-categories", description = "Expense category management"),
        (name = "vendors", description = "Vendor management"),
        (name = "expenses", description = "Expense tracking and approval"),
        (name = "realtime", description = "Realtime WebSocket and room management"),
        (name = "inventory", description = "Inventory locations, stock, transfers, and waste"),
        (name = "procurement", description = "Purchase orders, receiving, reorder controls, and stock movement ledger"),
        (name = "notifications", description = "Persisted notifications and activity log"),
    )
)]
pub struct ApiDoc;
