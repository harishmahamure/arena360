...
import { getExpectedClosing, handoverShift } from '../services/shifts';
...
export default function ShiftHandoverDialog({ open, onClose }: ShiftHandoverDialogProps) {
  ...
  const completeHandover = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Shift handover failed';
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
      <DialogTitle>End Shift & Handover</DialogTitle>
      <DialogContent>
        ...
        {step === 0 && (
          <Box>
            ...
            {variance !== 0 && (
              <Alert severity={Math.abs(variance) > 0.01 ? 'warning' : 'success'} sx={{ mt: 2 }}>
        ...
        {step === 2 && (
          <Box>
            ...
            <FormTextField
              ...
              value={validatorTotp}
              onChange={(event) => setValidatorTotp(event.target.value)}
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        {step < 2 ? (
          ...
          <FormButton
            ...
            disabled={
              loading || !validatorUsername || !validatorPassword || validatorTotp.length !== 6
            }
          >
            Complete Handover
          </FormButton>
